...
from utils.data_processor_pos_only import InputFeatures, convert_single_example, OreProcessor
...
def filed_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        feature = convert_single_example(ex_index, example, pos_mat, label_map, max_seq_length, tokenizer,
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, labels, pos_embedding, num_labels, use_one_hot_embeddings):  # 这里是构建模型的重点，需要改变
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        (total_loss, logits, predicts) = create_model(  # 使用BERT的接口建模
            bert_config, is_training, input_ids, input_mask, segment_ids, label_ids, pos_embedding,
...
def main(_):
    ...
    run_config = tf.contrib.tpu.RunConfig(
        ...
        master=FLAGS.master,
        model_dir=FLAGS.pos_output_dir,
    ...
    if FLAGS.do_train:
        train_file = os.path.join(FLAGS.pos_output_dir, "train.tf_record")
    ...
    if FLAGS.do_eval:
        ...
        eval_file = os.path.join(FLAGS.pos_output_dir, "eval.tf_record")
        ...
        output_eval_file = os.path.join(FLAGS.pos_output_dir, "eval_results.txt")
        ...
        output_predict_file = os.path.join(FLAGS.pos_output_dir, "label_test.txt")

    if FLAGS.do_predict:
        token_path = os.path.join(FLAGS.pos_output_dir, "token_test.txt")
        ...
        predict_file = os.path.join(FLAGS.pos_output_dir, "predict.tf_record")
