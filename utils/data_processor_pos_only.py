...
class InputExample(object):
    ...
    def __init__(self, guid, text, pos_embedding, label=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, pos_embedding):
    ...
    def _read_data(cls, input_file):  # 这里是对文件的处理
        ...
        with open(input_file, encoding='utf-8') as f:
            ...
            for line in f:
                ...
                poss = line['pos_seq']
                lines.append([labels, words, poss])
...
class OreProcessor(DataProcessor):
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExample(guid=guid, text=text, label=label, pos_embedding=pos_embedding))
        return examples
...
def convert_single_example(ex_index, example, pos_mat, label_map, max_seq_length, tokenizer, mode):  # 此时已经确定为最大长度
